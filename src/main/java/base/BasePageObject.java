...
public class BasePageObject extends TestUtilities {
	...
	protected boolean click(WebElement el) {
		...
		for (int i = 0; i < 2; i++) { // 2 tries to make tap
			try {
				waitFor(ExpectedConditions.elementToBeClickable(el),Duration.ofSeconds(30));
	...
	protected boolean m_click(WebElement el) {
		...
		for (int i = 0; i < 3; i++) { // 2 tries to make tap
			try {
				waitFor(ExpectedConditions.elementToBeClickable(el),Duration.ofSeconds(30));
				el.click();
				sleep(200);
...
//	}
	...
	protected String getText(WebElement element) {
		...
		while (attempts < 2) {
			try {
				waitFor(ExpectedConditions.visibilityOf(element),Duration.ofSeconds(30));
	...
	protected void waitFor(ExpectedCondition<WebElement> condition, Duration timeOutInSeconds) {
		timeOutInSeconds = timeOutInSeconds != null ? timeOutInSeconds : Duration.ofSeconds(0);
